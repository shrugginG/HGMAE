...
def main(args):
    ...
    (
        (
            nei_index,  # P-A, P-S
            feats,  # node features [<author>[7169,7167], <paper>[4019,1902], <subject>[60,60]]
            mps,  # [pap, psp], spare matrix
            pos,  # nihao
            label,  # [4019,3]
            # NOTE - train, test, val is only use from downstream task
            idx_train,
            idx_val,
            idx_test,
        ),
    ...
    nb_classes = label.shape[-1]  # number of class: 3
    feats_dim_list = [
        i.shape[1] for i in feats
    ]  # the dimension of features [1902, 7167, 60]

    num_mp = int(len(mps))  # number of meta-paths: 2
    print("Dataset: ", args.dataset)
    print("The number of meta-paths: ", num_mp)

    # Whether to use metapath2vec as the initial embedding
    if args.use_mp2vec_feat_pred:
        ...
        metapath_model = MetaPath2Vec(
            ...
            processed_metapaths,
            args.mps_walk_length,  # 10
            args.mps_context_size,  # 5
            args.mps_walks_per_node,  # 3
            args.mps_num_negative_samples,  # 3
            sparse=True,
        )
    ...
    # model
    focused_feature_dim = feats_dim_list[0]  # TODO -  target node feats: 1902? not 1966?
    ...
    feats = [feat.to(args.device) for feat in feats]  # All features
    ...
    if args.task == "classification":
        ...
        for i in range(len(idx_train)): # Training with differnet train 
